...
### Expanding an SMQ into dictionary terms

`smq_content.asc` lists SMQ members at the Preferred Term level (with a broad or
narrow scope), and some SMQs are hierarchical, referencing subordinate SMQs.
Adverse events, however, are coded at the Lowest Level Term level. `join_smq()`
bridges that gap: it resolves sub-SMQs recursively, expands each Preferred Term
down to all of its LLTs, and attaches the full hierarchy (via `join_meddra()`),
giving you the term list needed to merge against coded adverse events and to
build SMQ\* or CQ\* flags.
...
The result has one row per (SMQ × Lowest Level Term × SOC path). `term_scope` is
`1` (broad) or `2` (narrow). A common follow-on keeps only broad-scope, current
LLTs:
...
The columns use the same `snake_case` names as `join_meddra()`. If you need the
short SAS/ADaM variable names often used in clinical SMQ derivation, rename them:

| `join_smq()` column | SAS/ADaM | | `join_smq()` column | SAS/ADaM |
|--------|------|---|--------|------|
| `soc_code` | `soccd` | | `pt_code` | `ptcd` |
| `soc_name` | `soc` | | `pt_name` | `pt` |
| `hlgt_code` | `hlgtcd` | | `llt_code` | `lltcd` |
| `hlgt_name` | `hlgt` | | `llt_name` | `llt` |
| `hlt_code` | `hltcd` | | | |
| `hlt_name` | `hlt` | | | |

```{r smq-rename}
old <- c("soc_code", "soc_name", "hlgt_code", "hlgt_name", "hlt_code",
         "hlt_name", "pt_code", "pt_name", "llt_code", "llt_name")
new <- c("soccd", "soc", "hlgtcd", "hlgt", "hltcd", "hlt", "ptcd", "pt",
         "lltcd", "llt")
names(smq_df)[match(old, names(smq_df))] <- new
names(smq_df)
```
